...
class SimpleState(cmogcore.SimpleState):
    ...
    def __init__(self, turn: Turn=BLACK, owner_bits: int=0, hand_bits: int=0, promoted_bits: int=0,
        ...
        try:
            ...
            raise ValueError('invalid state: %s: %s' % (e, SimpleState.__repr_string(
    ...
    @staticmethod
    def wrap(state: cmogcore.SimpleState):
        return SimpleState(
            Turn(state._turn),
            state.owner_bits,
            state.hand_bits,
    ...
    def __repr_string(turn, owner_bits, hand_bits, promoted_bits, unused_bits, board, position):
        ...
        return 'SimpleState(turn=%r, %s, board=%r, position=[%s])' % (turn, flags, board, p)
...
class SimpleStateBuilder:
    ...
    def result(self):
        return SimpleState.wrap(self.__state)
