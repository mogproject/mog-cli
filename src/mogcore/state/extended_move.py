...
class ExtendedMove(cmogcore.ExtendedMove, TimedMove):
    ...
    def __init__(self, turn: Turn, from_: Pos, to: Pos, piece_type: PieceType, elapsed_time: Optional[int]=None):
        ...
        cmogcore.ExtendedMove.__init__(self, turn.value, from_.value, to.value, piece_type.value, et, 0, 0)
    ...
    def __repr__(self):
        return 'ExtendedMove(turn=%s, from_=%s, to=%s, piece_type=%s, elapsed_time=%s)' % (
    ...
    def from_string(cls, s: str):
        ...
        pattern = re.compile(r'^([+\-%#][1-9A-Z_]+)(?:,T(\d{1,5}))?$')
        mt = pattern.match(s)
        if not mt:
        ...
        if mv[:1] == '%' or mv[:1] == '#':
            ...
            m = Move.from_string(mv)
            return ExtendedMove(m.turn, m.from_, m.to, m.piece_type, t)
