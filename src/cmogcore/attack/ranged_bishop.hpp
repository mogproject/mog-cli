...
struct BishopAttackGenerator {
  template <int... Is>
  static constexpr auto generate(util::seq<Is...>) -> util::Array<decltype(&BishopAttack<false, 0>::get_attack), sizeof...(Is)> {
 ...
 */
template <bool Promoted>
struct BishopAttackTableGenerator {
  template <int... Is>
  static constexpr auto generate(util::seq<Is...>) -> util::Array<decltype(&BishopAttack<false, 0>::Base::get_variation), sizeof...(Is)> {
    return {{&BishopAttack<Promoted, Is>::Base::get_variation...}};
  }
