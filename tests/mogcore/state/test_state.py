...
class TestState(unittest.TestCase):
    ...
    def test_legal_moves(self):
        self.maxDiff = None
        f = lambda s: SimpleMove.from_string(s)
        ...
        self.assertEqual(p3.legal_moves(),
                         [SimpleMove(BLACK, HAND, Pos(i), ROOK) for i in range(81) if i != 4] +
                         [SimpleMove(BLACK, HAND, Pos(i), BISHOP) for i in range(81) if i != 4] +
                         [SimpleMove(BLACK, HAND, Pos(i), LANCE) for i in range(9, 81)] +
                         [SimpleMove(BLACK, HAND, Pos(i), SILVER) for i in range(81) if i != 4] +
                         [SimpleMove(BLACK, HAND, Pos(i), KNIGHT) for i in range(18, 81)] +
                         [SimpleMove(BLACK, HAND, Pos(i), PAWN) for i in range(9, 81)] +
                         [SimpleMove(BLACK, HAND, Pos(i), GOLD) for i in range(81) if i != 4]
                         )

        p4 = State.from_string('\n'.join([
            'P1 *  *  *  *  *  *  *  *  * ',
            'P2 *  *  *  *  *  *  *  *  * ',
            'P3 *  *  *  *  *  *  *  *  * ',
            'P4 *  *  * -KE * -OU *  *  * ',
        ...
        self.assertEqual(len(State(STATE_MAX_LEGAL_MOVES).legal_moves()), 593)
    ...
    def test_hash_value(self):
        ...
        self.assertEqual(State(SimpleState()).hash_value, 0)

        self.assertNotEqual(State(SimpleState().set_turn(WHITE.value)).hash_value, 0)

        self.assertEqual(State(SimpleState().set_turn(WHITE.value).set_turn(BLACK.value)).hash_value, 0)

        self.assertNotEqual(State(STATE_TSUME_BLACK).hash_value, State(STATE_TSUME_WHITE).hash_value)
        ...
        for st in gen_state(100):
            s.append(State(st).hash_value)

        self.assertEqual(len(s), 100)
        self.assertEqual(len(set(s)), 100)

    def test_is_checked(self):
        self.assertFalse(State(STATE_HIRATE).is_checked())
    ...
    def test_is_king_alive(self):
        self.assertFalse(State(SimpleState()).is_king_alive(BLACK.value))
        self.assertFalse(State(SimpleState()).is_king_alive(WHITE.value))
        self.assertTrue(State(STATE_HIRATE).is_king_alive(BLACK.value))
        self.assertTrue(State(STATE_HIRATE).is_king_alive(WHITE.value))
        self.assertFalse(State(STATE_TSUME_BLACK).is_king_alive(BLACK.value))
        self.assertTrue(State(STATE_TSUME_BLACK).is_king_alive(WHITE.value))
        self.assertTrue(State(STATE_TSUME_WHITE).is_king_alive(BLACK.value))
        self.assertFalse(State(STATE_TSUME_WHITE).is_king_alive(WHITE.value))
